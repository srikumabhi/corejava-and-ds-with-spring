package com.corejava.practice.javapractices.basicsofjava.threads.javaconcurrency.java7.forkjoinframework;
 ...
 *
 */
public class ForkJoinframeworkImpl extends RecursiveTask<List<String>> {
	...
	public ForkJoinframeworkImpl(String path, String extension)
	{
		this.path = path;
	...
	protected List<String> compute()
	{
		...
		//ForkJoinframeworkImpl tasks to store the subtasks that are going to process the subfolders stored in the folder
		List<ForkJoinframeworkImpl> tasks = new ArrayList<ForkJoinframeworkImpl>();
		//Get the content of the folder.
		File file = new File(path);
		File content[] = file.listFiles();
		//For each element in the folder, if there is a subfolder, create a new ForkJoinframeworkImpl object 
		...
		if (content != null)
		{
			for (int i = 0; i < content.length; i++)
			{
				...
				{
					ForkJoinframeworkImpl task = new ForkJoinframeworkImpl(content[i].getAbsolutePath(), extension);
					task.fork();
					tasks.add(task);
				}
		...
		//If the list of the ForkJoinframeworkImpl subtasks has more than 50 elements, 
	...
	private void addResultsFromTasks(List<String> list, List<ForkJoinframeworkImpl> tasks)
	{
		for (ForkJoinframeworkImpl item : tasks)
		{
