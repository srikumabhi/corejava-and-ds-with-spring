 ...
 *
 */
public class DifferenceBetweenArrayListAndLinkedListInJava {
	/*
	 * 1. Implementation : ArrayList is the resizable array implementation of 
	 * list interface , while LinkedList is the Doubly-linked list implementation 
	 * of the list interface.
       2. Performance : Performance of ArrayList and LinkedList depends on the 
       type of operation
       a. get(int index) or search operation : ArrayList get(int index) 
       operation runs in constant time i.e O(1) while LinkedList get(int index) operation run time is O(n) .
       The reason behind ArrayList being faster than LinkedList is that 
       ArrayList uses index based system for its elements as it internally uses array data structure , on the other hand ,
